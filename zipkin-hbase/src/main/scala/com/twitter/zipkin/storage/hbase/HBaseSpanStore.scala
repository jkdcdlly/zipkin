 ...
 */
class HBaseSpanStore(conf: Configuration) extends SpanStore with CollectAnnotationQueries {
  ...
  def getTraceIdsByName(
                         serviceName: String,
                         spanNameOption: Option[String],
                         endTs: Long,
                         lookback: Long,
                         limit: Int
                         ): Future[Seq[IndexedTraceId]] = {
    val resultsFuture = spanNameOption match {
      case Some(x) => getTraceIdsByNameWithSpanName(serviceName, x, endTs * 1000, lookback * 1000, limit)
      case None => getTraceIdsByNameNoSpanName(serviceName, endTs * 1000, lookback * 1000, limit)
    }
  ...
  def getTraceIdsByAnnotation(
                               serviceName: String, annotation: String, value: Option[ByteBuffer], endTs: Long, lookback: Long, limit: Int
                               ): Future[Seq[IndexedTraceId]] = {
   ...
   */
  override def getAllServiceNames: Future[List[String]] = serviceMapper.getAll.map { f => f.map(_.name).toList.sorted }

  /**
   * Get all the span names for a particular service, as far back as the ttl allows.
   */
  override def getSpanNames(service: String): Future[List[String]] = {
    // From the service get the spanNameMapper.  Then get all the maps.
    val spanNameMappingsFuture = serviceMapper.get(service).flatMap {
      _.spanNameMapper.getAll
    }
    // get the names from the mappings.
    spanNameMappingsFuture.map { maps => maps.map {
      _.name
    }.toList.sorted
    }
  }
  ...
  private[hbase] def indexSpanNameByService(span: Span): Future[Unit] = {
    ...
    Future.collect(serviceMappingsFuture.map { smf =>
      smf.flatMap {
        _.spanNameMapper.get(span.name)
      }
    }).flatMap {
      snm => Future.Unit
    }
  ...
  private[this] def getTraceIdsByNameNoSpanName(serviceName: String, endTs: Long, lookback: Long, limit: Int): Future[Seq[Result]] = {
    val serviceMappingFuture = serviceMapper.get(serviceName)
    serviceMappingFuture.flatMap { serviceMapping =>
      ...
      val startTs: Long = endTs - lookback
      val startRk = Bytes.toBytes(serviceMapping.id) ++ getEndScanTimeStampRowKeyBytes(startTs)
  ...
  private[this] def getTraceIdsByNameWithSpanName(serviceName: String, spanName: String, endTs: Long, lookback: Long, limit: Int): Future[Seq[Result]] = {
    val serviceMappingFuture = serviceMapper.get(serviceName)
    serviceMappingFuture.flatMap { serviceMapping =>
      ...
      spanNameMappingFuture.flatMap { spanNameMapping =>
        val scan = new Scan()
        val startTs: Long = endTs - lookback
        val startRow = Bytes.toBytes(serviceMapping.id) ++ Bytes.toBytes(spanNameMapping.id) ++ getEndScanTimeStampRowKeyBytes(startTs)
  ...
  private[this] def resultToSpans(option: Option[Result]): List[Span] = {
    val lists: List[KeyValue] = option match {
      case Some(result) => result.list().asScala.toList
      case None => List.empty[KeyValue]
    }

    val spans: List[Span] = lists.map { kv =>
