...
class HBaseSpanStoreSpec extends ZipkinHBaseSpecification {
  ...
  val traceId = 100L
  val spanId = 567L
  val span = Span(traceId, "span.methodCall()", spanId)
  ...
  test("getTraceIdsByName") {
    ...
    t1.map {
      _.traceId
    } should contain(traceIdOne)
    t1.map {
      _.timestamp
    } should contain(spanOneStart)
    t1.size should be(1)
    ...
    t2.map {
      _.traceId
    } should contain(traceIdFour)
    t2.map {
      _.timestamp
    } should contain(spanTwoStart)
    t2.map {
      _.timestamp
    } should contain(spanThreeStart)
  ...
  test("getTraceIdsByAnnotation") {
    ...
    ids.map {
      _.traceId
    } should contain(spanFive.traceId)
  }
  ...
  test("getTracesByIds") {
    Await.result(spanStore.apply(Seq(span)))
    val spansFoundFuture = spanStore.getTracesByIds(Seq(traceId, 302L))
